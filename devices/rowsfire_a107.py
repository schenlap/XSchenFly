...
class XP_Websocket:
    ...
    def dataref_id_fetch(self, dataref):
        ...
        if xpdr_code_response.status_code != 200:
            print(
                f"could not get id for {dataref}, Errorcode: {xpdr_code_response.status_code}:{xpdr_code_response.text}")
            return None
    ...
    def command_id_fetch(self, command):
        ...
        if xpdr_code_response.status_code != 200:
            print(
                f"could not get id for {command}, Errorcode: {xpdr_code_response.status_code}:{xpdr_code_response.text}")
            return None
        return xpdr_code_response.json()["data"][0]["id"]
    ...
    def dataref_set_value(self, id, value, index=None):
        ...
        if index != None:
            xpdr_code_response = self.xp.patch(self.rest_url + "/datarefs/" + str(id) + "/value",
                                               data=json.dumps(set_msg), params={"index": index})
        else:
            xpdr_code_response = self.xp.patch(self.rest_url + "/datarefs/" + str(id) + "/value",
        ...
        if xpdr_code_response.status_code != 200:
            print(
                f"could not set data for id {id}. Errorcode: {xpdr_code_response.status_code}:{xpdr_code_response.text}")
            return None
    ...
    def command_activate_duration(self, id, duration=0.2):
        ...
        if xpdr_code_response.status_code != 200:
            print(
                f"could not send command for id {id}. Errorcode: {xpdr_code_response.status_code}:{xpdr_code_response.text}")
            return None
...
class DREF_TYPE(Enum):
    ...
    ARRAY_30 = 40
    ARRAY_31 = 41
    DATA_MULTIPLE = 100  # more leds use the same dataref. Changed to 100 to make room for higher ARRAY_x values -mrivorey
...
def create_led_list_a107():  # TODO check sim/cockpit/electrical/avionics_on == 1
    ...
    ledlist.append(
        Led("EMERGENCY EXIT LIGHT OFF", MF_SR2, 22, "AirbusFBW/OHPLightsATA31_Raw", DREF_TYPE.ARRAY_12, "==0"))
    ledlist.append(Led("BAT1_VOLTAGE", MF_SEGMENT1, [16, 56], "AirbusFBW/BatVolts", DREF_TYPE.ARRAY_0,
                       "int(($+0.05)*10)"))  # round fist decimal
    ledlist.append(
        Led("BAT2_VOLTAGE", MF_SEGMENT1, [2, 7], "AirbusFBW/BatVolts", DREF_TYPE.ARRAY_1, "int(($+0.05)*10)"))
...
def create_button_list_a107():
    ...
    buttonlist.append(
        Button("Beacon Light", MF_MP3, 2, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_0, BUTTON.TOGGLE_INVERSE))
    buttonlist.append(
        Button("Wing Light", MF_MP3, 3, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_1, BUTTON.TOGGLE_INVERSE))
    buttonlist.append(
        Button("Nav Light 1", MF_MP3, 5, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_2, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Nav Light 2", MF_MP3, 4, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_2, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Land Left 1", MF_MP3, 8, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_4, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Land Left 2", MF_MP3, 7, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_4, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Land Right 1", MF_MP3, 10, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_5, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Land Right 2", MF_MP3, 9, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_5, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Nose Light 1", MF_MP3, 11, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_3, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Nose Light 2", MF_MP3, 12, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_3, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("RWY Turn Light", MF_MP3, 6, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_6, BUTTON.TOGGLE_INVERSE))
    buttonlist.append(
        Button("Strobe 1", MF_MP3, 1, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_7, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Strobe 2", MF_MP3, 0, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_7, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Seatbelt", MF_MP3, 13, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_11, BUTTON.TOGGLE_INVERSE))
    buttonlist.append(
        Button("Ice Eng1 On", MF_MP1, 1, "toliss_airbus/antiicecommands/ENG1On", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(
        Button("Ice Eng1 Off", MF_MP1, 1, "toliss_airbus/antiicecommands/ENG1Off", DREF_TYPE.CMD, BUTTON.SWITCH))
    buttonlist.append(
        Button("Ice Eng2 On", MF_MP1, 0, "toliss_airbus/antiicecommands/ENG2On", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(
        Button("Ice Eng2 Off", MF_MP1, 0, "toliss_airbus/antiicecommands/ENG2Off", DREF_TYPE.CMD, BUTTON.SWITCH))
    buttonlist.append(
        Button("Ice Wing On", MF_MP1, 2, "toliss_airbus/antiicecommands/WingOn", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(
        Button("Ice Wing Off", MF_MP1, 2, "toliss_airbus/antiicecommands/WingOff", DREF_TYPE.CMD, BUTTON.SWITCH))
    buttonlist.append(
        Button("Pack1 On", MF_MP1, 7, "toliss_airbus/aircondcommands/Pack1On", DREF_TYPE.CMD, BUTTON.SWITCH))
    buttonlist.append(
        Button("Pack1 Off", MF_MP1, 7, "toliss_airbus/aircondcommands/Pack1Off", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(
        Button("Pack2 On", MF_MP1, 5, "toliss_airbus/aircondcommands/Pack2On", DREF_TYPE.CMD, BUTTON.SWITCH))
    buttonlist.append(
        Button("Pack2 Off", MF_MP1, 5, "toliss_airbus/aircondcommands/Pack2Off", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(Button("Fire APU", MF_MP2, 0, "sim/electrical/APU_fire_shutoff", DREF_TYPE.CMD,
                             BUTTON.SWITCH_INVERSE))  # does not work
    buttonlist.append(
        Button("Fire Eng1", MF_MP2, 1, "AirbusFBW/ENGFireSwitchArray", DREF_TYPE.ARRAY_0, BUTTON.SWITCH_INVERSE))
    buttonlist.append(
        Button("Fire Eng2", MF_MP3, 15, "AirbusFBW/ENGFireSwitchArray", DREF_TYPE.ARRAY_1, BUTTON.SWITCH_INVERSE))
    buttonlist.append(Button("Fire Test APU", MF_MP2, 4, "AirbusFBW/FireTestAPU", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(
        Button("Fire Test Eng1", MF_MP2, 5, "AirbusFBW/FireTestENG1", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(
        Button("Fire Test Eng2", MF_MP2, 3, "AirbusFBW/FireTestENG2", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(
        Button("Adirs ON Bat", MF_MP1, 14, "sim/annunciator/clear_master_warning", DREF_TYPE.CMD, BUTTON.TOGGLE))
    buttonlist.append(
        Button("Smoking Light 1", MF_MP4, 3, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_12, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Smoking Light 2", MF_MP3, 14, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_12, BUTTON.SWITCH_COMBINED))
    buttonlist.append(Button("Calls", MF_MP2, 2, "AirbusFBW/purser/fwd", DREF_TYPE.CMD, BUTTON.HOLD))
    buttonlist.append(
        Button("Adirs 1-1", MF_MP4, 6, "AirbusFBW/ADIRUSwitchArray", DREF_TYPE.ARRAY_0, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Adirs 1-2", MF_MP4, 7, "AirbusFBW/ADIRUSwitchArray", DREF_TYPE.ARRAY_0, BUTTON.SWITCH_COMBINED))
    # buttonlist.append(Button("Adirs 1-3", MF_MP4, 9, "AirbusFBW/ADIRUSwitchArray", DREF_TYPE.ARRAY_0, BUTTON.SEND_2)) # not needed
    buttonlist.append(
        Button("Adirs 2-1", MF_MP4, 10, "AirbusFBW/ADIRUSwitchArray", DREF_TYPE.ARRAY_1, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Adirs 2-2", MF_MP4, 11, "AirbusFBW/ADIRUSwitchArray", DREF_TYPE.ARRAY_1, BUTTON.SWITCH_COMBINED))
    # buttonlist.append(Button("Adirs 2-3", MF_MP4, 8, "AirbusFBW/ADIRUSwitchArray", DREF_TYPE.ARRAY_1, BUTTON.SEND_2)) # not needed
    buttonlist.append(
        Button("Adirs 3-1", MF_MP4, 8, "AirbusFBW/ADIRUSwitchArray", DREF_TYPE.ARRAY_2, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Adirs 3-2", MF_MP4, 9, "AirbusFBW/ADIRUSwitchArray", DREF_TYPE.ARRAY_2, BUTTON.SWITCH_COMBINED))
    ...
    buttonlist.append(
        Button("Bat1 Off", MF_MP2, 15, "toliss_airbus/eleccommands/Bat1Off", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(Button("Bat2 On", MF_MP2, 14, "toliss_airbus/eleccommands/Bat2On", DREF_TYPE.CMD, BUTTON.SWITCH))
    buttonlist.append(
        Button("Bat2 Off", MF_MP2, 14, "toliss_airbus/eleccommands/Bat2Off", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(
        Button("APU Gen On", MF_MP1, 8, "AirbusFBW/ElecOHPArray", DREF_TYPE.ARRAY_2, BUTTON.SWITCH))  # does not work
    buttonlist.append(
        Button("IR1", MF_MP1, 13, "sim/flight_controls/brakes_1_auto", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(
        Button("IR2", MF_MP1, 12, "sim/flight_controls/brakes_2_auto", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(
        Button("IR3", MF_MP1, 11, "sim/flight_controls/brakes_max_auto", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(
        Button("ExtPwrON", MF_MP2, 13, "toliss_airbus/eleccommands/ExtPowOn", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
    buttonlist.append(
        Button("ExtPwrOFF", MF_MP2, 13, "toliss_airbus/eleccommands/ExtPowOff", DREF_TYPE.CMD, BUTTON.SWITCH))
    ...
    buttonlist.append(
        Button("Exit 1", MF_MP4, 5, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_10, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Exit 2", MF_MP4, 4, "AirbusFBW/OHPLightSwitches", DREF_TYPE.ARRAY_10, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Wiper 1", MF_MP4, 15, "AirbusFBW/LeftWiperSwitch", DREF_TYPE.DATA, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Wiper 2", MF_MP4, 14, "AirbusFBW/LeftWiperSwitch", DREF_TYPE.DATA, BUTTON.SWITCH_COMBINED))
    buttonlist.append(
        Button("Flap 3 Off", MF_MP1, 15, "toliss_airbus/gpwscommands/Flap3Off", DREF_TYPE.CMD, BUTTON.SWITCH))
    buttonlist.append(
        Button("Flap 3 On", MF_MP1, 15, "toliss_airbus/gpwscommands/Flap3On", DREF_TYPE.CMD, BUTTON.SWITCH_INVERSE))
        ...
        if ref_id in led_dataref_ids:
            ...
            if type(value) is list:  # dataref array, ledlist array
                ...
                for v in value:
                    ...
                    idx += 1
            elif type(ledobj) == list and type(
                    value) != list:  # multiple leds on same dataref (without dataref arry), for eval
                for l in ledobj:
